...
def retrieve(Bmode, offset, interval, wrapper_file):
	...
	if Bmode == "b":
		...
		while offset < len(wrapper_bytes):
			...
			offset+=interval
		#Remove the last 6 bytes as those will be the sentinel bytes and we don't want those to render in our final extracted file
		H = H[0:len(H)-6]
...
def main():
	...
	if mode == "r":
		extracted_file = retrieve(Bmode, offset, interval, wrapper_file)
